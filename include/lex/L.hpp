...
namespace ast
{
    ...
    public:
        ...
        explicit Lexeme(const StmtLoc *_Loc, long double f, const Tok &_t)
        :Loc(_Loc), data({.s = "", .i = 0, .f = f}),tok(_t){}
        explicit Lexeme(const StmtLoc *_Loc, int64_t i,  const Tok &_t)
        :Loc(_Loc), data({.s = "", .i = i, .f = 0.0}),tok(_t){}
        ~Lexeme(){}
        ...
        String getStr() const noexcept { return data.s; }
    ...
    private:
        ...
        StmtLoc *getLoc() noexcept;
       
        Token_type getKeyWord();
        bool PutToken();
        bool LexConstString(String &Str);
        bool LexConstChar();
        bool SkipWhiteSpace();
        bool SkipCommentLine();
        bool SkipCommentBlock();
        
        bool LexConstNumerical(Token_type &type);
        
        bool RemoveSlashes(String &s);
    public:
        Lexer(ResourceMgr &_mgr, Module *_mod, String &_Src, Vec<Lexeme> &_LxmS) 
